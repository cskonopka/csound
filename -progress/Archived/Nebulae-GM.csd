...
;		kTrig4		metro	kSHTrig
	...
	; Speed
	kOsc1Trig ctrl14 1, 1, 2, 0, 13

	; Pitch
	kOsc2Trig ctrl14 1, 15, 16, 0, 13
	...
	; Glide
	kOsc3Trig ctrl7 1, 70, 0, 13

	; Mix
	initc7 1, 8, 1
	kOscFreq ctrl7 1, 8, 0, 220
	
	; Grain Rate
	kFM1dex ctrl7 1, 5, 0, 5

	; Grain Size
	kFM1mod ctrl7 1, 4, 0, 5

	; Loop Start
	kFM1freq ctrl14 1, 9, 10, 0, 120

	; Loop Size
	kFM1car ctrl14 1, 11, 12, 0, 5
		
				kTrig1   	metro   kOsc1Trig
		kTrig2		metro	kOsc2Trig
		kTrig3		metro	kOsc3Trig
		
		;aOsc5 	oscil 	kOscGroupAmp, (kOscGroupFreq*0.089), giGsine
		;aOsc4 	oscil 	kOscGroupAmp, (kOscGroupFreq*0.055), giRecaman4
		;aOsc3 	oscil 	kOscGroupAmp, (kOscGroupFreq*0.034), giGsine
		;aOsc2 	oscil 	kOscGroupAmp, (kOscGroupFreq*0.021), giGsine
		;aOsc1 	oscil 	kOscGroupAmp, (kOscGroupFreq), giMersenneSine
...
;			aLFOo1			lfo 	kLFOamp, (kLFOfreq*0.55), 1			
			...
			aOsc1			oscili	.5, kOscFreq, giGsine
...
;			aLFOo2			lfo 	kLFOamp, (kLFOfreq*0.89), 2			
			...
			aOsc2			oscili	.5, kOscFreq*0.089, giRecaman2
...
;			aLFOo3			lfo 	kLFOamp, kLFOfreq, 5			
			...
			aOsc3			oscili	.5, kOscFreq*0.055, giGsine
			iGaussRand3 	random 	0.2, 6.3
			aGaussDel3 		delay 	aNoise3, iGaussRand3
			aOscTrig3 		= 		(aOsc3+(aGaussDel3*0.45))	
...
;		aSH 		samphold aOsc, kTrig4
		...
		aFM 	foscili			.9, kFM1freq, kFM1car, kFM1mod, kFM1dex, giMersenneSine
