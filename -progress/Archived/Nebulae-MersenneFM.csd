/*
~ Nebulae_MersenneFM.csd ~

	This is an FM instrument that is influenced by 3 individual oscillator triggers. 
	The tables for this instrument were created using the Mersenne and Recaman number
	sequences. 

	Mersenne: http://mathworld.wolfram.com/MersenneNumber.html
	Recaman:  http://mathworld.wolfram.com/RecamansSequence.html
...
instr 2
	...
	; ~ gkOsc1Trig ~ [Speed]
	;	Controls the trigger rate of its associated if-statement
	gkOsc1Trig 	ctrl14 1, 1, 2, 0, 3

	; ~ gkOsc2Trig ~ [Pitch]
	;	Controls the trigger rate of its associated if-statement
	gkOsc2Trig 	ctrl14 1, 15, 16, 0, 3

	; ~ gkOsc3Trig ~ [Glide]
	;	Controls the trigger rate of its associated if-statement
	gkOsc3Trig ctrl7 1, 70, 0, 3

	; ~ gkOscFreq ~ [Mix]
	;	Ocillator master frequency
			initc7 1, 8, 1
	gkOscFreq 	ctrl7 1, 8, 0.1, 32
	
	; ~ gkFM1dex ~ [Grain Rate]
	;	FM index
	gkFM1dex ctrl7 1, 5, 0.01, 5

	; ~ gkFM1mod ~ [Grain Size]
	;	FM modulator
	gkFM1mod ctrl7 1, 4, 0.01, 5

	; ~ gkFM1freq ~ [Loop Start]
	;	FM frequency
	gkFM1freq ctrl14 1, 9, 10, 4, 128

	; ~ gkFM1car ~ [Loop Size]
	;	FM carrier
	gkFM1car ctrl14 1, 11, 12, 0.1, 5

	; Oscillator metronomes
	kTrig1   	metro   gkOsc1Trig
	kTrig2		metro	gkOsc2Trig
	kTrig3		metro	gkOsc3Trig
		
	; Oscillator group
	aOsc5 		oscil 	.1, (gkOscFreq*0.089), giGsine
	aOsc4 		oscil 	.1, (gkOscFreq*0.055), giRecaman2
	aOsc3 		oscil 	.1, (gkOscFreq*0.034), giGsine
	aOsc2 		oscil 	.1, (gkOscFreq*0.021), giRecaman1
	aOsc1 		oscil 	.1, (gkOscFreq), giMersenneSine
		
	aOscGroup 	=		(aOsc1*aOsc2*aOsc3*aOsc4*aOsc5)
	
	; Trigger 1
	if(kTrig1 == 1)then		
		aNoise1			gauss	(.02 * gkFM1dex)
		ares 			areson 	aNoise1, 600, 1150 
		aOsc1			oscili	.2, gkOscFreq, giRecaman3
	...
	; Trigger 2	
	if(kTrig2 == 1)then	
		aNoise2			gauss	(.04* gkFM1dex)
		ares2 			areson 	aNoise2, 550, 70 
		aOsc2			oscili	.2, gkOscFreq*0.089, gisine
		iGaussRand2 	random 	0.8, 6.3
		aGaussDel2 		delay 	ares2, .2841
		aOscTrig2 		= 		(aOsc2+(aGaussDel2)) 
	endif
		
	; Trigger 3	
	if(kTrig3 == 1)then			
		aNoise3			gauss		(.03* gkFM1dex)
		ares3 			areson 	aNoise3, 1200, 1800 
		aOsc3			oscili	.2, gkOscFreq*0.055, giRecaman4
		iGaussRand3 	random 	0.8, 6.3
		aGaussDel3 		delay 	ares3, .1712	
		aOscTrig3 		= 		(aOsc3+(aGaussDel3))	
	endif				

	; Grouped Triggers
	aOsc 	= 			((aOscTrig1*.3)+(aOscTrig2*0.3)+(aOscTrig3*0.3))+aOsc5+aOsc4


	; Main FM out
	aFM 	foscili		.5, gkFM1freq, gkFM1car, gkFM1mod, gkFM1dex, giMersenneSine
	
	ares balance aFM, aOsc
