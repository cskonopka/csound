...
; [Nebulae Controls]
	...
	; ~ CombReverb ~ 	[Mix]
				initc7 	1, 8, 1
	kVerbMix 		ctrl7 	1, 8, 0.001, 100
	kVerbMix 		portk 	kVerbMix, 0.08	

	; ~ Frequency ~ 		[Loop Start]
	kFrequency 		ctrl14 	1, 9, 1, 0.001, 1400	
	...
	kBnkOSCf2 		ctrl14 	1, 11, 12, 0.001, 100
	kBnkOSCf2 		portk 	kBnkOSCf2, 0.08	

	; ~ Low Shift ~ 			[Grain Rate]
	kLowShift 		ctrl7 	1, 4, 0.001, 400
	kLowShift 		portk 	kLowShift, 0.08
				
		kosc oscil .2, kVerbMix, gisine
		
		kosc1 oscil .3, kBnkOSCf2+(kVerbMix+40), giRecaman
		kosc2 oscil .3, kBnkOSCf2+(kVerbMix+10), giRecaman				
		kosc3 oscil .3, kBnkOSCf2+(kVerbMix+80), giRecaman
		kosc4 oscil .3, kBnkOSCf2+(kVerbMix+30), giRecaman
						
		aosc1 oscil .4, (kFrequency+kBnkOSCf1)*kosc1+8, giRecaman
		aosc2 oscil .4, (kFrequency+kBnkOSCf1+31)*kosc2+2, giRecaman5
		aosc3 oscil .4, (kFrequency+kBnkOSCf1+8)*kosc3+6, giRecaman4
		aosc4 oscil .4, (kFrequency+kBnkOSCf1+10)*kosc4+4, giRecaman3
		
		aosc = aosc1+aosc2+aosc3+aosc4
		aComb combinv aosc, .2, .1 
...
fsig    pvsanal 	aComb*.9, ifftsize, ioverlap, iwinsize, iwinshape

fs 		pvswarp 	fsig, kENVstretch, 1, (4000+kosc)-(10*kLowShift), 1, .5, 1024
