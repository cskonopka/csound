...
; [Nebulae Controls]
	; ~ ENV Stretch ~ 		[Speed]
	kOf1 	ctrl14 	1, 1, 2, 0.001, 1.5
	kOf1 	portk 	kOf1, 0.08

	; ~ Cutoff ~ 			[Pitch]
	kS1 		ctrl14 	1, 15, 16, 0.001, 2
	kS1 		portk 	kS1, 0.08	
...
adel      oscil   .1, kOf1*kMf, 1

adel    =  sin (2 *3.14159265 * adel) *kS1 + 0.81 
adel2a    =  sin (2 *3.14159265 * adel) *((kS1+2.1)-kS3) + 1.81 
aenv2 linseg 0, .8, .3
        deltapxw a1, adel+adel2a, 64

adel    oscil   .3, kOf2*kMf, 1
adel    =  sin (2 * 3.14159265  * adel) * kS2 + 0.21
adel2b    =  sin (2 *3.14159265 * adel) *((kS2+2.1)-kS1) + 1.47 
aenv2 linseg 0, .8, .3
        deltapxw a1*aenv2, adel+adel2b, 64


adel    oscil   .5, kOf3*kMf, 1
adel    =  sin (2 * 3.14159265  * adel) * kS3 + 0.5
adel3b    =  sin (2 * 3.14159265  * adel) *((kS3+2.1)-kS2) + 0.5
aenv3 linseg 0, .8, .8
aenv3a linseg .8, .8, .1

adel3 clip adel, 1, 0.6

        deltapxw (a1*aenv3), adel+adel3b, 64
